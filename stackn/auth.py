...
# STACKN_SECURE is by default always set to True.
# However we allow to deploy STACKn locally for development and
# testing purposes.
# That's when a user needs to login with the flag --insecure,
# and thus STACKN_SECURE will be False
# The util function make sure to remind the user to use --insecure if set
# in the ~/.scaleout/stackn.json configuration file
def _check_flag_insecure(inp_conf):
    stackn_conf_file = _load_config_file_full({})

    # If the general default STACKN_SECURE is still True,
    # but in the configuration file shows that
    # it is set to False instead (because the user has logged in
    # with --insecure)
    # Then we remind the user to use --insecure
    if (
        not stackn_conf_file["current"]["STACKN_SECURE"]
        and inp_conf["STACKN_SECURE"]
    ):
        out_str = (
            "Do you perhaps have a local STACKn deployment? "
            "If so, don't forget to use --insecure to make stackn "
            "API calls working."
        )
        print(out_str)
        return True
...
def _is_user_logged():
    ...
    if not stackn_conf_file:
        ...
        out_str = (
            ...
            "stackn login -u <user> -p <password> --url <studio-url>"
        )
        print(out_str)
        return False
    else:
    ...
    if stackn_config_full:
        try:
            ...
            if conf["STACKN_URL"] and not is_login:
                print(
                    "Failed to load config file: {} not set up.".format(
                        conf["STACKN_URL"]
                    )
                )
                print(e)
...
def _set_current(conf):
    ...
    if current["STACKN_URL"]:
        ...
        if current["STACKN_PROJECT"]:
            stackn_config["current"]["STACKN_PROJECT"] = current[
                "STACKN_PROJECT"
            ]
        else:
    ...
    if (
        "STACKN_URL" in os.environ
        and stackn_config["current"]["STACKN_URL"] != ""
    ):
        print(
            "STACKN_URL set as environment variable and this takes priority."
        )
        print(
            "Set by 'export STACKN_URL={}'".format(
                stackn_config["current"]["STACKN_URL"]
            )
        )
    if (
        "STACKN_PROJECT" in os.environ
        and stackn_config["current"]["STACKN_PROJECT"] != ""
    ):
        print("Using STACKN_PROJECT set as environment variable.")
        print(
            "Set by 'export STACKN_PROJECT={}'".format(
                stackn_config["current"]["STACKN_PROJECT"]
            )
        )
    if (
        "STACKN_SECURE" in os.environ
        and stackn_config["current"]["STACKN_SECURE"] != ""
    ):
        print("Using STACKN_SECURE set as environment variabley.")
        print(
            "Set by 'export STACKN_SECURE={}'".format(
...
def get_config(
    inp_config=dict(), required=[], is_login=False, print_warnings=True
):
    # Order of priority:
    # 1. Values in inp_config
    # 2. Environment variables
    # 3. Config file
    # Exception is STACKN_ACCESS_TOKEN, and STACKN_REFRESH_TOKEN,
    # where config file
    # takes priority over environment variable (as they need to be updated)

    # Checking if the user is logged in.
    # Skipping if when the user needs to login instead
    ...
    for var, val in env_vars.items():
        if var in inp_config and inp_config[var] is not None:
    ...
    if "STACKN_SECURE" in current and (
        ...
        if not print_warnings and current["STACKN_SECURE"]:
            print(
                "Insecure mode is set in config, will not verify certificates."
            )
...
def get_token(conf={}):
    ...
    if "http" not in conf["STACKN_URL"]:
...
def stackn_login(
    studio_url=[], client_id=[], username=[], password=[], secure=True
):
    ...
    # adding flag in conf for informing that set_current
    # is begin invoked by stackn_login
