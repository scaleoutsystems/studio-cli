...
def call_project_endpoint(name, conf={}, params=[]):
    ...
    if not status:
        out_str = (
            "The configuration file for STACKn "
            "could not be correctly retrieved."
        )
        print(out_str)
        return False
...
def setup_project_endpoint_call(conf, endpoint_type):
    ...
    if not project:
        ...
        out_str = (
            "Found several matching projects. "
...
def get_endpoints(studio_url):
    # These endpoints are related to the API available in Studio under:
    # stackn/components/studio/api/
    endpoints = dict()
...
def get_auth_header(conf):
    ...
    if not status:
        return False, False
    auth_header = {
        "Authorization": "Token {}".format(conf["STACKN_ACCESS_TOKEN"])
    }
...
def create_app(
    ...
    _ = subprocess.run(
        ["tar", "-C", chart_archive, "-czvf", chart_uid, "."],
...
def validate_template(template, conf):
...
def create_project(
    ...
    template_valid = validate_template(template, conf)
...
def create_meta_resource(filename, studio_url, project, secure):
    ...
    project = get_projects(
        conf=conf["STACKN_URL"],
        params={"name": conf["STACKN_PROJECT"]},
...
def create_object(
    ...
    r = requests.post(
        url, json=model_data, headers=auth_header, verify=secure_mode
    )

    if not _check_status(r, error_msg="Failed to create model."):
        # Delete model object from storage.
        # repo.delete_artifact(model_uid) # repo is not defined
        return False
    ...
    print(
        "Released model: {}, release_type: {}".format(model_name, release_type)
    )
...
def create_appinstance(studio_url=[], project=[], data={}, secure_mode=True):
    ...
    if not conf or not auth_header or not url:
        ...
        return False
    res = requests.post(
        url, headers=auth_header, data=data, verify=conf["STACKN_SECURE"]
    )
    print(res.text)
...
def delete_meta_resource(
    resource_type, name, project=[], studio_url=[], secure=True
):
    conf = {
        "STACKN_PROJECT": project,
        "STACKN_URL": studio_url,
    ...
    if not prj_endpts:
        ...
        print(
            "No {}: '{}' associated with the current project".format(
                resource_type, name
            )
        )
        return False
