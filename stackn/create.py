...
def app(settings, chart, logo, studio_url, secure):
    create_app(
        settings, chart, logo=logo, studio_url=studio_url, secure_mode=secure
    )
...
def apps(studio_url, secure):
    create_apps(studio_url=studio_url, secure_mode=secure)


@create.command("appinstance")
@click.option(
    "-u", "--studio-url", required=False, default=[], help="Studio URL"
)
@click.option(
    "-p", "--project", required=False, default=[], help="Project name"
)
@click.option("--secure/--insecure", default=True)
@click.argument("file")
def appinstance(studio_url, project, secure, file):
    ...
    create_appinstance(
        studio_url=studio_url, project=project, data=data, secure_mode=secure
    )
...
def templates(studio_url, secure):
    create_templates(studio_url=studio_url, secure_mode=secure)
...
@create.command(
    "meta-resource",
    help="Such as: project environments, flavors, mlflow and s3 enpoints",
)
...
def metaresource(filename, studio_url, project, secure):
    create_meta_resource(
        filename, studio_url=studio_url, project=project, secure=secure
    )
